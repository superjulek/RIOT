...
int ipsec_ts_from_pkt(gnrc_pktsnip_t *pkt, ipsec_ts_t *ts)
{
    ...
    if ((udp = gnrc_pktsnip_search_type(pkt, GNRC_NETTYPE_UDP)))
    {
        ...
        tmp_ts.src_port = ntohl(udp_hdr->src_port.u16);
        tmp_ts.dst_port = ntohl(udp_hdr->dst_port.u16);
    }
#endif
    ...
    if ((tcp = gnrc_pktsnip_search_type(pkt, GNRC_NETTYPE_TCP)))
    {
        ...
        tmp_ts.src_port = ntohl(tcp_hdr->src_port.u16);
        tmp_ts.dst_port = ntohl(tcp_hdr->dst_port.u16);
    }
#endif
    *ts = tmp_ts;

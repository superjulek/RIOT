...
{
    size_t len = sizeof(ike_generic_payload_header_t) + + sizeof(ike_identification_payload_const_t) + id.len;
    if (max_len < len)
    {
        return -ENOMEM;
    };
...
int process_identification_payload(char *start, size_t max_len, size_t *cur_len, ike_payload_type_t *next_payload, ike_id_type_t *id_type, chunk_t *id)
{
    ...
    if (ret < 0)
        return ret;
    ike_identification_payload_const_t *ph;
    if (*cur_len < sizeof(ike_generic_payload_header_t) + sizeof(ike_identification_payload_const_t))
    {
        return -ENOMEM;
    }
    size_t id_len = *cur_len - sizeof(ike_generic_payload_header_t) - sizeof(ike_identification_payload_const_t);
    ph = (ike_identification_payload_const_t*)(start + sizeof(ike_identification_payload_const_t));
    *id_type = ph->id_type;
    free_chunk(id);
    *id = malloc_chunk(id_len);
    memcpy(id->ptr, start + sizeof(ike_generic_payload_header_t) + sizeof(ike_identification_payload_const_t), id_len);
    return 0;
}
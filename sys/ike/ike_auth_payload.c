...
int process_auth_payload(char *start, size_t max_len, size_t *cur_len, ike_payload_type_t *next_payload, ike_auth_method_t *auth_method, chunk_t *auth_data)
{
    ...
    size_t auth_len = *cur_len - sizeof(ike_generic_payload_header_t) - sizeof(ike_auth_payload_const_t);
    ike_auth_payload_const_t *ph;
    if (ret < 0)
        return ret;
    ...
    if (auth_data->len != auth_len)
        return -1;
    memcpy(auth_data->ptr, start + sizeof(ike_generic_payload_header_t) + sizeof(ike_auth_payload_const_t), auth_len);
